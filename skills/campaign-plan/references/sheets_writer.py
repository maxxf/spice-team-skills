...
def write_full_tab(sheet_id: str, tab: str, matrix: list[list[Any]],
    ...
    # --- write safety ----------------------------------------------------------
    # The clear below is destructive and runs BEFORE the `if not matrix` check
    # further down — so an empty matrix wipes the tab and writes nothing. That is
    # exactly what blanked goop's Sheet on 2026-06-16. Snapshot, then gate, then
    # clear. Order matters.
    dry_run = dry_run or _WRITE_MODE["dry_run"]
