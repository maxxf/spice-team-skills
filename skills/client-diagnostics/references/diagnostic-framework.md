...
- **Watch**: error rate 2–5%, OR cancellation 2–5%, OR uptime 90–97%, OR rating 4.2–4.5, OR an **intentional/capacity downtime event** (see below)
- **Broken**: error rate >5%, OR cancellation >5%, OR uptime <90%, OR rating <4.2, OR repeated hours-mismatch incidents, OR an **involuntary downtime event** (see below)

> **Discrete downtime EVENTS override the smoothed averages.** A 90-day run
> averages a discrete outage down to noise — a store DoorDash literally
> auto-paused can still show 99% average uptime and score "Healthy" if you only
> read the single uptime %. You MUST read the DoorDash downtime export **by
> category** (`Downtime Category - Short` / `Description`) and classify events,
> not just the average:
>
> | DoorDash downtime category | Nature | Verdict |
> |---|---|---|
> | `Auto Pause - High Avoidable and/or POS Cancellation Rate` | Involuntary — platform pulled the store offline | **Broken → Red** |
> | `Store Closure - Dasher Reported` | Involuntary — dasher found the store closed/unresponsive | **Broken → Red** |
> | `Auto Pause - High Avoidable Dasher Wait Time` | Capacity — kitchen throughput at peak | **Watch** |
> | `OLO Merchant Triggered - Temporary Store Closure` | Intentional — merchant chose to pause | **Watch** (NOT Red unless combined with an involuntary Broken event) |
>
> Supporting signal: `Avoidable Store Operations` cancellations (DoorDash
> cancellations export → "Store closed" / "Store operations issue") corroborate
> an involuntary failure but are not, on their own, the trigger. An involuntary
> event forces Broken **regardless of** a healthy average uptime/error/rating —
> the "Ops Broken always wins → Red" rollup rule then fires because the event
...
> **Promo count is NOT a tier determinant.** A stacked promo mix (≥2 concurrent
> promos) used to force a "Watch" — it no longer does. Over-discounting is
> surfaced as a **NOTED margin observation** (in `campaigns_detail` and a margin
> note), never a tier flag. A store can run 3–5 promos and still be Green if its
> ROAS + incrementality are healthy. ROAS is computed **spend-weighted** (blended
> = total attributed / total spend), not as a plain mean of per-row ROAS, so a
> zero-spend platform (e.g. a store running no Grubhub ads) doesn't drag the
> store into the Watch band.
