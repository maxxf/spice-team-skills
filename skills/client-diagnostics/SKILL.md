...
- **DD downtime export (categorized) → ops-event columns.** From the `Downtime Category - Short` / `Description` + `Minutes Downtime` columns, sum minutes per store per category into the optional event columns (put the store's window total on one representative row, `0` elsewhere — the ops sub-skill SUMs them):
  - `Auto Pause - High Avoidable and/or POS Cancellation Rate` → `auto_pause_involuntary_min` (Broken → Red)
  - `Store Closure - Dasher Reported` → `dasher_closure_min` (Broken → Red)
  - `Auto Pause - High Avoidable Dasher Wait Time` → `dasher_wait_pause_min` (Watch)
  - `OLO Merchant Triggered - Temporary Store Closure` → `merchant_closure_min` (Watch)
  - Cancellations export `Avoidable Store Operations` (Store closed / Store operations issue) → `avoidable_ops_cancels` (supporting evidence)
  - Do NOT collapse downtime to a single uptime %: discrete involuntary events must survive into these columns so the ops bucket can flag Red even when average uptime looks healthy.
- **DD sponsored listings + promos** → spend, attributed_sales, roas, promo_count_active per store. Note: `promo_count_active` is a NOTED margin observation only — it never determines a store's tier.
