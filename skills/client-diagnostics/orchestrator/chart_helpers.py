"""Diagnostic chart functions used by the orchestrator + sub-skills.

Refactored from `references/generate_diagnostic_charts.py` (preserved as backup).
Each function takes a `metrics` dict and an `out` directory `Path`, writes a PNG
inside that directory using a fixed filename, and returns the resulting path.

Wk 3 ships these 4 functions wired through the pipeline:
    - radar_7dim
    - tier_donut
    - top15_green_bar
    - campaign_2x2

The other 4 functions (sparklines_gmv_orders, funnel_ue, top_skus_bar,
daypart_heatmap) are preserved here for future enrichment but are not yet
called by the orchestrator — they need richer input than synthetic data
currently provides (weekly buckets, impression-stage funnel, per-SKU revenue,
day-of-week × hour cells respectively).
"""
