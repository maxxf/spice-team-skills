...
- [ ] **Tax-exclusion sanity** — verify AOV is in the typical $20-$50 range for the client. If AOV jumped 7-10% with no menu change OR `commissions_pct` is below the platform floor (UE <27%, DD <25%, GH <22%), Total Sales likely includes tax. Re-extract using the pre-tax column (see methodology §2a). Validator should have flagged this in the QA section — if it didn't, that's a bug to fix.
