...
name: campaign-ops
description: >
  Full campaign ops pipeline: brief, register in Campaign Planning DB (single source
  of truth), create Task Tracker ticket, assign ops analyst, flag design needs to
  Dilli, enforce naming conventions, require screenshot proof, notify #support-ops.
  Every campaign MUST go through this skill. Trigger on "set up campaign for [client]",
  "brief a campaign", "BOGA/BOGO for [client]", "set up promo", "launch offer on
  UE/DD/GH", "campaign ticket", "run [offer] at [locations]", "set up ads", "schedule
  featured listing", "DashPass offer", "Grubhub loyalty", "what campaigns are running",
  "campaign status for [client]", or any request to set up a delivery marketplace
  campaign. Covers Uber Eats, DoorDash, and Grubhub. For EXECUTION, not strategy
  (use campaign-planner for strategy).
---
...
- **Slack MCP** (send notifications to #support-ops, #int-[client], and #design-campaigns)
...
**A. Post to #support-ops** (channel ID: C08DJUDFVHR) — the implementation handoff:
...
**B. Post to the client's internal channel** (`#int-[client]`) — awareness for the account team:

First, search Slack for the client's internal channel. Pattern: `#int-[client-slug]`
(e.g., `#int-goop-kitchen`, `#int-fresh-kitchen`, `#int-pret`). Use `slack_search_channels`
with the client name to find it. If no channel exists, skip this step and note it in the
confirmation to the user.

Send a leaner, context-focused message:

```
:satellite_antenna: *New Campaign Briefed*
[Campaign Type]: [Offer summary] ([Platform(s)])
:round_pushpin: [N] locations | [Start date] → [End date]
[If design needed] :art: Design: Dilli ([asset type])
[If not approved] :warning: Pending client approval
:link: [Campaign Planning registry entry URL]
```

This is NOT a task assignment. It's visibility. The GM, Maxx, anyone touching the account
sees what's going live without digging through #support-ops.

**C. Post to #design-campaigns** if design/creative is needed:

Only send this if `Needs design/creative = Yes`. This channel is where designers
(Dilli, Fabio, Michelle, Morena) pick up campaign creative work.

Search Slack for `#design-campaigns`. Send:

```
:art: *New Campaign Creative Request*
...
*Brief:*
[1-2 sentence creative direction from user's input]

*Specs:* [platform-specific dimensions if known]
*Deadline:* [start date minus 2 days, or "ASAP" if start is <3 days out]

:link: [Campaign Planning registry entry URL] (full brief + upload deliverables here)
```
...
4. Confirmation that Slack notifications were sent (#support-ops + #int-[client])
5. If design needed: confirmation that #design-campaigns was notified
6. Approval status (approved or pending)
7. Design status (if applicable: Dilli assigned, asset type)
