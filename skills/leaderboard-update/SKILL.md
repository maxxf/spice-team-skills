...
description: >
  Weekly update of the Everytable Store Leaderboard Excel workbook.
  Rotates current data to Previous Week, loads new platform exports (UE order history + ratings,
  DD financial transactions + operations quality, GH operations review), recalculates rankings.
  Trigger on "update leaderboard", "leaderboard update for Everytable", "load leaderboard data",
  "weekly leaderboard", "refresh leaderboard", "new week leaderboard", or when the user provides
  Everytable platform exports and mentions the leaderboard.
---

# Everytable Store Leaderboard — Weekly Update

Update the Everytable Store Leaderboard with new weekly platform data. This skill handles the full rotation: archive current data as Previous Week, load new exports, and let the formulas do the rest.

---

## Prerequisites

The workbook lives at: `mnt/Everytable/Everytable_Store_Leaderboard.xlsx`
Structure: 5 tabs (Instructions, Leaderboard, Previous Week, Data Input, Stores)
The Leaderboard tab is 100% formula-driven. You only touch Data Input, Previous Week, and the period label.
