...
- `references/client-comms-pass.md` is the checking procedure. Mechanical pass/fail on length, ask position, link presence, and banned patterns, then the voice check, then the one line of specific human attention.
- `skills/client-email/SKILL.md` is the front door for the team. One sentence of intent in, a canon-shaped draft in the teammate's own Gmail drafts. It never sends.
...
The four skills that emit client-facing copy (`post-client-meeting`, `ratings-reply`, `client-onboarding`, `email-template-designer`) run the pass before they emit anything. `sales-follow-up` lives in HQ rather than in this plugin, so it is covered by the mirrored HQ policy `spice-client-comms-brief-exec-shaped` instead.
