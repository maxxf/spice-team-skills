...
Run from inside the `campaign-plan` skill directory (in a Cowork session, cwd is the skill folder by default; if running manually, `cd` to wherever Cowork installed the `spice-marketplaces` plugin's `skills/campaign-plan/`):
