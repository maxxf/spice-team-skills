...
6. **Experiment-data freshness (accountability gate)** — every 🟡 Running / 🔵 Read-due row in the **Experiments** tab must have a reading logged and `Data thru (wk)` stamped to the **current week** by end of run. A Running experiment whose data wasn't updated this week is a CRITICAL gap — the read at its decision week is being built on missing interim data. Flag it against its named **owner** (`⚠ EXP-## data not updated — owner <name>`), and never let a test be marked 🟢 Concluded without the interim readings behind it. This is what makes "we tracked the experiment" true rather than assumed — the test-vs-control number is captured *every* week between Start and Read, not reconstructed at the end.
