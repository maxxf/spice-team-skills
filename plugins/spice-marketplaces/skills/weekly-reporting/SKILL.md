...
**2. Tracker update — WRITE IN PLACE (preferred).** Write the week's values directly into the client's **canonical** Campaign Tracker. **Never create a new spreadsheet and never Drive-copy the tracker** — a `_W##` copy fragments the source of truth and strands plan edits (this is the #1 failure mode of this skill). See `references/sheets-writer.md` for the recipe.
   ...
   - Update each tab by its own rule (do NOT treat it like a single paste column):
     ...
     - **Experiments** — the in-flight test register. The weekly run *does* maintain its status column: advance ⚪ Planned→🟡 Running once `Start` week has passed, and mark 🔵 **Read due** any row whose `Read / decide` week ≤ the current week and `Status` ≠ Concluded. **Surface every Read-due row in the report** (see Experiments read-watch below) so a decision date never slips. Only fill `Result` / set 🟢 Concluded when the GM has made the call; on conclusion, append the durable finding to **Account Learnings**.
   ...
   - **Experiments read-watch:** before finishing, read the tracker's **Experiments** tab. For any row with `Read / decide` week ≤ current week and not Concluded, add a top-of-report callout — `⏰ EXP-##: <test> — read due <week>. Decision rule: <rule>. This week's reading: <pull the relevant test-vs-control number>.` This is the mechanism that turns a buried `READ:` plan-cell into an on-time decision. If the client has no Experiments tab, skip silently.
