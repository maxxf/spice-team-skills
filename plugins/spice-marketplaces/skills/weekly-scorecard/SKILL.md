...
description: >
  Spin up the Spice Weekly Scorecard live artifact in this user's Cowork sidebar,
  OR run the bridge script to produce scorecard_rows.csv from weekly-reporting
  skill outputs, OR generate a static client-facing HTML export. Trigger on
  "show me the scorecard", "open the weekly scorecard", "spin up the scorecard",
  "scorecard for [client]", "generate scorecard rows", "build client scorecard
  export", or any request to view, fill, or share the weekly scorecard.
  Owner is Santi. Reporters are Manish and Dulari. GMs tune thresholds and write
  Signal callouts.
---
